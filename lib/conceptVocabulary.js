...
function afFieldHaystackRaw(fieldMeta) {
  return [fieldMeta.name, fieldMeta.id, fieldMeta.placeholder, fieldMeta.label, fieldMeta.ariaLabel]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
}
 ...
 * Short tokens (≤3 chars) require word boundaries on the raw (spaced) haystack
 * so "url" does not match inside random strings, and "name" is less greedy.
 */
function afKeywordMatches(rawHaystack, compactHaystack, keyword) {
  ...
  if (compactKw.length <= 3) {
    const escaped = compactKw.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const re = new RegExp(`(^|[^a-zа-яё0-9])${escaped}([^a-zа-яё0-9]|$)`, "i");
    return re.test(rawHaystack);
  }

  // Multi-word keywords: allow both spaced raw and compact forms.
  if (kw.includes(" ") && rawHaystack.includes(kw)) return true;
